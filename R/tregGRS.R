tregGRS <-
function(query.p, reference.p, query.gL=NULL, reference.gL=NULL, na.rm=TRUE, 
	estimateNullDistr=TRUE, nullDistrQuantiles=c(0.9, 0.95, 0.99), nullDistrN=100, 
	tolerateWarnings=TRUE, pAdjust.method.query=NULL, pAdjust.method.reference=NULL, lambda=0.005,rescale=T) {
	...
	if(rescale){
		...
		sNorm<-normalize.quantiles(cbind(s1,s2))
		s1<-sNorm[,1]
		s2<-sNorm[,2]
		pNorm<-normalize.quantiles(cbind(p1,p2))
		p1<-pNorm[,1]
		p2<-pNorm[,2]
	}
